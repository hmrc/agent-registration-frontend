...
extends FrontendController(mcc, actions):
  ...
  def show(linkId: LinkId): Action[AnyContent] =
    actions
      ...
      .ensure(
        condition = _.get[RiskingOutcomeApplication.FailedFixable].reSubmittedAt.isEmpty,
        resultWhenConditionNotMet = _ => Redirect(AppRoutes.providedetails.riskingoutcome.RiskingOutcomeController.show(linkId))
      ):
        implicit request =>
