...
extends FrontendController(mcc, actions):
  ...
  def signOut: Action[AnyContent] = actions.deleteMeAction:
  ...
  def timeOut: Action[AnyContent] = actions.deleteMeAction:
    val continueUrl = uri"${appConfig.thisFrontendBaseUrl + AppRoutes.SignOutController.timedOut.url}"
    signOutWithContinue(continueUrl.toString)

  def timedOut: Action[AnyContent] = actions.deleteMeAction:
    implicit request =>
      Ok(timedOutPage())
