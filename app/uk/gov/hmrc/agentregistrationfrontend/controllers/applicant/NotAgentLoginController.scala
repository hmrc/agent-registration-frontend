...
@Singleton
class NotAgentLoginController @Inject() (
  mcc: MessagesControllerComponents,
  defaultActionBuilder: DefaultActionBuilder,
  af: AuthorisedFunctions,
  errorResults: ErrorResults,
  appConfig: AppConfig,
  view: NotAgentLoginPage
...
extends FrontendControllerBase(mcc),
  ...
  def show(continueUrl: Option[RedirectUrl]): Action[AnyContent] = defaultActionBuilder.async:
    implicit request =>
      af.authorised(AuthProviders(GovernmentGateway)).apply:
        ...
        val signInUrl =
          appConfig
            .signInUri(uri"$absoluteContinueUrl", AffinityGroup.Agent)
            .toString
