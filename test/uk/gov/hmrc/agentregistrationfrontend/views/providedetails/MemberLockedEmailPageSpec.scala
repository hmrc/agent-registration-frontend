...
class MemberLockedEmailPageSpec
extends ViewSpec:

  val memberLockedEmailPage: MemberEmailLockedPage = app.injector.instanceOf[MemberEmailLockedPage]
  ...
  given memberProvideDetailsRequest: MemberProvideDetailsRequest[AnyContent] = tdAll.makeProvideDetailsRequest(memberProvidedDetails.afterEmailAddressProvided)

  val doc: Document = Jsoup.parse(
    memberLockedEmailPage().body
  )
