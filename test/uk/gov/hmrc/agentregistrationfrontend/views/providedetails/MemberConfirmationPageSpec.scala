...
class MemberConfirmationPageSpec
extends ViewSpec:

  val viewTemplate: MemberConfirmationPage = app.injector.instanceOf[MemberConfirmationPage]
  implicit val memberProvideDetailsRequest: MemberProvideDetailsRequest[AnyContent] = tdAll.makeProvideDetailsRequest(
    memberProvidedDetails = tdAll.providedDetailsLlp.afterHmrcStandardforAgentsAgreed
  )
  val doc: Document = Jsoup.parse(
    viewTemplate(applicantName = "Test Applicant", companyName = "Test Company Name").body
  )

  "MemberConfirmationPage" should:
