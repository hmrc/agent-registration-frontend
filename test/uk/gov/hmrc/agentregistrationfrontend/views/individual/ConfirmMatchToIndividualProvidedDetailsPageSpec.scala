...
class ConfirmMatchToIndividualProvidedDetailsPageSpec
extends ViewSpec:
  ...
  "ConfirmMatchToIndividualProvidedDetailsPage" should:
    ...
    "render a radio button for each option" in:
      val expectedRadioGroup: TestRadioGroup = TestRadioGroup(
        legend = yesNoLabel,
        options = List(
          "Yes, these details are correct" -> YesNo.Yes.toString,
