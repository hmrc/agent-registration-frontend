...
extends ViewSpec:
  ...
  "AmlsEvidenceUploadPage view" should:
    ...
    "render config values correctly for JavaScript to use when enabled" in:
      ...
      ) shouldBe AppRoutes.fixablefailures.amlsfailure.AmlsEvidenceUploadController.checkUploadStatusJs.url
      progressIndicator.attr(
        "data-success"
      ) shouldBe s"$thisFrontendBaseUrl${AppRoutes.fixablefailures.amlsfailure.AmlsEvidenceUploadController.showUploadResult.url}"
