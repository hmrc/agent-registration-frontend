...
extends ControllerSpec:
  ...
  s"POST $path with invalid characters should return 400" in:
    ...
    val doc = response.parseBodyAsJsoupDocument
    doc.title() shouldBe "Error: What is your full name? - Apply for an agent services account - GOV.UK"
    doc.mainContent.select(s"#${AuthorisedNameForm.key}-error").text() shouldBe "Error: Your full name must only include letters a to z, hyphens, apostrophes and spaces"
