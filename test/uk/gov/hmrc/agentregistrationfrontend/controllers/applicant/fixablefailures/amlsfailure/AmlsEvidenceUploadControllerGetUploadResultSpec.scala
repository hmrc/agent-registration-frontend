...
extends ControllerSpec:
  ...
  private object agentApplication:
    ...
    val afterRiskingCompletedFixableNonHmrcAmls: AgentApplication =
      tdAll
        .agentApplicationLlp
        .afterRiskingCompletedFixableNonHmrcAmls

    val afterUploadSucceeded: AgentApplication = afterRiskingCompletedFixableNonHmrcAmls

  "routes should have correct paths and methods" in:

    AppRoutes.fixablefailures.amlsfailure.AmlsEvidenceUploadController.showUploadResult shouldBe Call(
      method = "GET",
  ...
  s"GET $uploadResultPath, if the Upload is in progress, should render appropriate message without updating any records" in:
    ...
    ApplyStubHelper.stubsToSupplyBprToPage(agentApplication.afterRiskingCompletedFixableNonHmrcAmls)
