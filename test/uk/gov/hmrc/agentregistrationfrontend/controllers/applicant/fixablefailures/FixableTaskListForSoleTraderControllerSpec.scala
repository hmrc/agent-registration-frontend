  ...
  object agentApplication:
    val riskingCompletedFixable: AgentApplicationSoleTrader =
      tdAll
        ...
        .riskingOutcomeSoleTraderAmls

  // these are the HTML document ids used in the fixable task list page
  object tasks:

    val amls = "amlsDetails-1-status" // only ever one task
    def soleTraderFailures(index: Int) = s"soleTraderFailures-$index-status" // multiple tasks possible, index starts at 1
