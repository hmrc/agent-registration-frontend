...
extends ControllerSpec:
  ...
  "routes should have correct paths and methods" in:
    arf.aboutyourbusiness.routes.PartnershipTypeController.show shouldBe Call(
      method = "GET",
      url = path
    )
    arf.aboutyourbusiness.routes.PartnershipTypeController.submit shouldBe Call(
      method = "POST",
      url = path
    )
    arf.aboutyourbusiness.routes.PartnershipTypeController.submit.url shouldBe arf.aboutyourbusiness.routes.PartnershipTypeController.show.url
  ...
  BusinessType.Partnership.values.foreach: partnershipType =>
    s"POST $path selecting a ${partnershipType.toString} should redirect to the sign in filter page" in:
      ...
      response.status shouldBe Status.SEE_OTHER
      response.body[String] shouldBe ""
      response.header("Location").value shouldBe arf.aboutyourbusiness.routes.TypeOfSignInController.show.url
